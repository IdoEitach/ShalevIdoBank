...
namespace ShalevIdoBank.DAL
{
    public class AccountServiceDal
    {
        ...
        public void ChangeDescription(int transactionId, string newDescription)
        {
            SqlCommand cmd = new SqlCommand("spChangeDescription", connection, transaction);
            cmd.Parameters.Add("@transactionId", SqlDbType.Int).Value = transactionId;
            cmd.Parameters.Add("@newDescription", SqlDbType.NVarChar).Value = newDescription;
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.ExecuteScalar();
        }
