...
namespace ShalevIdoBank.BLL
{
    public class AccountService
    {
        ...
        static public void ChangeTransactionDescription(string accountUserName, string accountPassword, int transactionId, string newDescription)
        {
            var service = new AccountServiceDal();
            if (!service.ValidateUser(accountUserName, accountPassword)) return;
            int accountId = service.GetAccountIdByUserName(accountUserName);

            service.ChangeDescription(transactionId, newDescription);
        }
